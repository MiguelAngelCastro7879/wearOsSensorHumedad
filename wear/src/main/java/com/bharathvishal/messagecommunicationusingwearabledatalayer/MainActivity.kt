...
class MainActivity : ComponentActivity(), AmbientModeSupport.AmbientCallbackProvider,
    ...
    override fun onSensorChanged(p0: SensorEvent?) {
        ...
        val Respuesta1: String = "Se encendió"
        val Respuesta2: String = "Se apagó"
        if(p0!!.values[0] < 30){
            binding.messagecontentEditText.text = Respuesta2.toEditable()
        }
        else{
            binding.messagecontentEditText.text = Respuesta1.toEditable()
        }
        binding.sendmessageButton.performClick()
    }

    override fun onAccuracyChanged(p0: Sensor?, p1: Int) {
        val Respuesta1: String = "A LA ESPERA"
        binding.messagecontentEditText.text = Respuesta1.toEditable()
    }
